...
# BEGIN MODULE1 V21 WATER OUTER CIRCLE THEN ROI

MODULE1_ANALYSIS_VERSION = "ACR_MODULE1_CT_NUMBER_QUICK_V21_WATER_OUTER_CIRCLE_THEN_ROI_2026_07_25"
...
def _m1v21_detect_outer_water_circle(raw, phantom_cx, phantom_cy, phantom_radius, inserts, water_roi_radius):
    """
    Main V21 idea:
      1. Find the faint OUTER water circle first.
      2. Then put the measurement ROI inside that outer circle.
    """
    ...
    target = _m1v21_expected_water_zone(
        phantom_cx=phantom_cx,
        phantom_cy=phantom_cy,
        phantom_radius=phantom_radius,
        inserts=inserts,
    ...
    if inserts:
        radii = [float(item.get("radius", 0.0)) for item in inserts if float(item.get("radius", 0.0)) > 0.0]
        base_radius = float(np.median(radii)) if radii else float(phantom_radius) * 0.105
    else:
        base_radius = float(phantom_radius) * 0.105

    min_outer_radius = max(8.0, base_radius * 0.75)
    max_outer_radius = min(float(phantom_radius) * 0.18, base_radius * 1.45)

    search_rx = float(phantom_radius) * 0.19
    search_ry = float(phantom_radius) * 0.20
    step_xy = max(3, int(round(base_radius * 0.16)))
    step_r = max(2, int(round(base_radius * 0.12)))
    ...
    y_min = int(max(max_outer_radius, ty - search_ry))
    y_max = int(min(h - max_outer_radius, ty + search_ry))
    x_min = int(max(max_outer_radius, tx - search_rx))
    x_max = int(min(w - max_outer_radius, tx + search_rx))

    for y in range(y_min, y_max + 1, step_xy):
        for x in range(x_min, x_max + 1, step_xy):
            if math.hypot(float(x) - tx, float(y) - ty) > max(search_rx, search_ry):
                continue

            for radius in np.arange(min_outer_radius, max_outer_radius + 0.1, step_r):
                radius = float(radius)

                if math.hypot(float(x) - float(phantom_cx), float(y) - float(phantom_cy)) + radius > float(phantom_radius) * 0.82:
                    continue

                if _m1v21_overlaps_insert(x, y, radius, inserts, safety=0.95):
                    continue

                outer_mask = _m1v21_circle_mask(raw.shape, x, y, radius)
                if line_bar_mask is not None and np.any(line_bar_mask[outer_mask]):
                    overlap_fraction = float(np.sum(line_bar_mask[outer_mask])) / max(float(np.sum(outer_mask)), 1.0)
                    if overlap_fraction > 0.01:
                        continue
                else:
                    overlap_fraction = 0.0

                roi_radius = min(float(water_roi_radius), radius * 0.55)
                stats = _m1v21_roi_stats(raw, x, y, roi_radius)
                if stats is None:
                    continue

                dog_arc = _m1v21_arc_score(dog, x, y, radius)
                grad_arc = _m1v21_arc_score(grad, x, y, radius)

                target_dist = math.hypot(float(x) - tx, float(y) - ty)
                layout_score = max(0.0, 100.0 - (target_dist / max(search_rx, 1e-6)) * 100.0)

                mean_hu = float(stats["mean"])
                sd_hu = float(stats["std"])

                water_quality = max(0.0, 60.0 - abs(mean_hu) * 2.0) + max(0.0, 50.0 - sd_hu * 2.4)
                arc_score = float(dog_arc["arcScore"]) * 2.5 + float(grad_arc["arcScore"]) * 0.35
                partial_bonus = max(int(dog_arc["partialArcCount"]), int(grad_arc["partialArcCount"])) * 5.0

                # The outer circle must be found first; then ROI fits inside.
                roi_fit_bonus = 25.0 if float(water_roi_radius) <= radius * 0.58 else 8.0

                quality = layout_score * 1.8 + water_quality + arc_score + partial_bonus + roi_fit_bonus

                candidates.append({
                    "x": round(float(x), 3),
                    "y": round(float(y), 3),
                    "outerRadius": round(float(radius), 3),
                    ...
                    "arcScore": round(float(arc_score), 3),
                    "partialArcBonus": round(float(partial_bonus), 3),
                    "roiFitBonus": round(float(roi_fit_bonus), 3),
                    "meanHU": round(float(mean_hu), 2),
                    "stdHU": round(float(sd_hu), 2),
                    "targetDistancePixels": round(float(target_dist), 3),
                    "lineBarOverlapFraction": round(float(overlap_fraction), 5),
                    "dogArc": dog_arc,
                    "gradArc": grad_arc,
                })
    ...
    if not candidates:
        return {
            ...
            "x": float(target["x"]),
            "y": float(target["y"]),
            "outerRadius": float(base_radius),
            "roiRadius": min(float(water_roi_radius), float(base_radius) * 0.55),
            "fallbackUsed": True,
            "expectedWaterTarget": target,
            "mapDiagnostics": map_diag,
    ...
    candidates = sorted(candidates, key=lambda item: float(item["quality"]), reverse=True)
    best = candidates[0]
...
# END MODULE1 V21 WATER OUTER CIRCLE THEN ROI
